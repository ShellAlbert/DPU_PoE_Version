...
//               output_En_DQ_i
///////////////////////////////////////////////////////////////////////////
//Register Repulication to reduce High Fanout problem of output_En_DQ_i.
//Fanout is reduced from 21 to 9.
reg [3:0] output_En_DQ_i; //tristate control.
wire [15:0] DQ_In;
wire [15:0] DQ_Out;
assign DQ_In=ioDQ; //Input.  //16'bzzzzzzzzzzzzzzzz
assign ioDQ[3:0]=(output_En_DQ_i[0])?(DQ_Out[3:0]):({4{1'bz}}); //Output.
assign ioDQ[7:4]=(output_En_DQ_i[1])?(DQ_Out[7:4]):({4{1'bz}}); //Output.
assign ioDQ[11:8]=(output_En_DQ_i[2])?(DQ_Out[11:8]):({4{1'bz}}); //Output.
assign ioDQ[15:12]=(output_En_DQ_i[3])?(DQ_Out[15:12]):({4{1'bz}}); //Output.
...
if(!iRstN) begin 
    ...
    output_En_DQ_i<=4'b1111; DQ_Out_Rising<=0; DQ_Out_Falling<=0; 
end
else begin
    if(iEn) begin
        case(iOpReq)
            ...
            3'b001: //[2:0]=3'b001, Read Registers.
                ...
                `STEP_00: //Read Identification Register 0, Pull CS# Low.
                    begin 
                        ...
                        CA_DR_i<=CA_IDENTIFICATION_REG0; step_i<=step_i+1; 
                    end
                `STEP_01:
                ...
                `STEP_04: //Pull CS# Low.
                    begin oCSN<=0; output_En_DQ_i<=4'b1111; step_i<=step_i+1; end
                `STEP_05: //The First-Three Clocks to send Command-Address.
                    if(cnt1_i==3-1) begin cnt1_i<=0; output_En_DQ_i<=4'b0000; additional_latency_i<=(iRWDS0)?1:0; step_i<=step_i+1; end
                    else begin cnt1_i<=cnt1_i+1; end
            ...
            3'b010: //[2:0]=3'b010, Write Registers.
                ...
                `STEP_00: //Read Identification Register 0, Pull CS# Low.
                    begin 
                        ...
                        CA_DR_i<=CA_CONFIG_REG0_WRITE; step_i<=step_i+1;
                    end
                `STEP_01:
                ...
                `STEP_04:  //Pull CS# Low. //RG[15:8],1_000_1111_1111_1_1_11=0x8FFF
                    begin 
                        oCSN<=0;   
                        output_En_DQ_i<=4'b1111; DQ_Out_Rising<={8'd0,CONFIG_REG0_WR_VALUE}; DQ_Out_Falling<={8'd0,CONFIG_REG0_WR_VALUE}; 
                        step_i<=step_i+1; 
                    end
                `STEP_05:
            ...
            3'b011: //[2:0]=3'b011, Read Memory.
                ...
                `STEP_00: 
                    begin 
                        ...
                        output_En_RWDS0<=0; output_En_RWDS1<=0;  
                        //prepare address.
                        //CA[47]=1, Read Transaction. CA[46]=0, Memory Space. CA[45]=1, Linear Burst. //3'b101
                        ...
                        CA_DR_i<={3'b101,9'd0,iOpMemAddr[22:8],iOpMemAddr[7:3],13'd0,iOpMemAddr[2:0]}; step_i<=step_i+1; 
                    end
                `STEP_01:
                    begin DQ_Out_Rising<={8'd0,CA_DR_i[47:40]}; DQ_Out_Falling<={8'd0,CA_DR_i[39:32]}; step_i<=step_i+1; end
                `STEP_02:
                ...
                `STEP_04: //Pull CS# Low.
                    begin oCSN<=0; output_En_DQ_i<=4'b1111; step_i<=step_i+1; end
                `STEP_05: //The First-Three Clocks to send Command-Address.
                    if(cnt1_i==3-1) begin cnt1_i<=0; output_En_DQ_i<=4'b0000; step_i<=step_i+1; end
                ...
                `STEP_08: //IDDRx1 needs 4 clocks to output data.
                    if(cnt1_i==4) begin cnt1_i<=0; step_i<=step_i+1; end
                        ...
                        cnt1_i<=cnt1_i+1;
                        //oRdData<={oRdData[95:0],DQ_In_Rising_D1,DQ_In_Falling_D1}; 
                        case(cnt1_i)
                        0: begin oRdData[127:96]<={DQ_In_Rising_D1,DQ_In_Falling_D1}; end
                        1: begin oRdData[95:64]<={DQ_In_Rising_D1,DQ_In_Falling_D1}; end
                        2: begin oRdData[63:32]<={DQ_In_Rising_D1,DQ_In_Falling_D1}; end
                        3: begin oRdData[31:0]<={DQ_In_Rising_D1,DQ_In_Falling_D1}; end
                        endcase
                    end
            ...
            3'b100: //[2:0]=3'b100, Write Memory.
                ...
                `STEP_00: 
                    begin 
                        ...
                        //[2:0]: Column Adress.
                        //3-bits+9-bits+15-bits+5-bits+13-bits+3-bits=48-bits.
                        CA_DR_i<={3'b001,9'd0,iOpMemAddr[22:8],iOpMemAddr[7:3],13'd0,iOpMemAddr[2:0]}; step_i<=step_i+1; 
                    end
                `STEP_01:
                    begin DQ_Out_Rising<={8'd0,CA_DR_i[47:40]}; DQ_Out_Falling<={8'd0,CA_DR_i[39:32]}; step_i<=step_i+1; end
                `STEP_02:
                ...
                `STEP_03:
                    begin DQ_Out_Rising<={8'd0,CA_DR_i[15:8]}; DQ_Out_Falling<={8'd0,CA_DR_i[7:0]}; step_i<=step_i+1; end
                `STEP_04: //Pull CS# Low.
                    begin oCSN<=0; output_En_DQ_i<=4'b1111; oClkUsed<=1; step_i<=step_i+1; end
                `STEP_05: //The First-Three Clocks to send Command-Address.
