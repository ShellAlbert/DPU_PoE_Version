...
int main() {
  ...
  // Set in/out baud rate to be 2 Mbps
  ...
  char buffer[64];
  sprintf(buffer,"dat-%d-%d",gRxCnt,fd_cnt++);
...

  {
    ...
    char read_buf [1024];
    strcpy(read_buf,"IloveChina!\r\n");
    while(1)
    {
      write(serial_port,read_buf,strlen(read_buf));
      usleep(100);
    }
